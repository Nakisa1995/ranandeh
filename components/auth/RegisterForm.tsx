...
export default function RegisterForm({ locale }: { locale: 'en' | 'fa' }) {
  ...
  const t = {
    ...
    cta: { en: 'Create account', fa: 'ایجاد حساب' },
    creating: { en: 'Creating…', fa: 'در حال ایجاد…' },
  };
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // تلاش برای لاگین خودکار با credentials
      ...
      if (signed?.ok) {
        ...
        // اگر credentials در دسترس نبود ولی API کوکی session ست کرده بود
        router.push(`/${locale}`);
      }
    } catch (e: any) {
      setErr(e?.message || (locale === 'fa' ? 'خطای سرور' : 'Server error'));
    } finally {
      setLoading(false);
    }
  }
    ...
    <form onSubmit={onSubmit} className="space-y-5" noValidate>
      ...
      >
        ...
        <span className="relative">
          {loading ? t.creating[locale] : t.cta[locale]}
        </span>
      </button>
    </form>
  );

...
import authOptions from '@/lib/auth'; // ← از default export استفاده می‌کنیم
import { mkdir, writeFile } from 'fs/promises';
import { join } from 'path';

// ⬇️ جلوگیری از collect در build + اجرای Node.js
...
export async function POST(req: Request) {
  ...
  // userId را از session می‌گیریم
  const userId =
    (session.user as any).id ||
    String(session.user.email || '').replace(/[^a-z0-9]/gi, '');
